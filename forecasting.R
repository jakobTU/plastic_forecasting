...
fcast_funcs_names <- c('HW', 'ETS', 'SARIMA', 'RF', 'ERT', 'XGBoost',
                       'GPR linear', 'GPR RBF')

fcast_funcs_exog_names <- c('SARIMAX', 'RF', 'ERT', 'XGBoost', 'GPR linear', 'GPR RBF')
...
# Add the Mean and Mean percent deviation rows to the matrix.
mat_rmse <- rbind(mat_rmse,
                  colMeans(mat_rmse),                           # Calculation of the Mean row.
                  colMeans(mat_rmse / apply(mat_rmse, 1, min))) # Calculation of the Mean percent deviation row.
rownames(mat_rmse) <- c(var_endog, 'Mean', 'Mean percent deviation')
...
mat_smape <- rbind(mat_smape,
                   ...
                   colMeans(mat_smape / apply(mat_smape[ , 1:8], 1, min))) # Calculation of the Mean percent deviation row (excluding Naive last).
rownames(mat_smape) <- c(var_endog, 'Mean', 'Mean percent deviation')
...
mat_rmse_exog <- foreach(fcast_func=fcast_funcs[3:8], .combine='cbind') %:%
...
mat_rmse_exog <- rbind(mat_rmse_exog,
                       ...
                       colMeans(mat_rmse_exog / apply(mat_rmse_exog, 1, min))) # Calculation of the Mean percent deviation row.
rownames(mat_rmse_exog) <- c(var_endog, 'Mean', 'Mean percent deviation')
colnames(mat_rmse_exog) <- fcast_funcs_exog_names


# Matrix of sMAPE values with incorporating external factors.
# See Table 8 in the paper.
mat_smape_exog <- foreach(fcast_func=fcast_funcs[3:8], .combine='cbind') %:%
  foreach(endog=var_endog, xreg=ts_exog, .combine='c') %do%
  model_eval(fcast_func, endog, res_to_smape, xreg)

# Add the Mean and Mean percent deviation rows to matrix.
mat_smape_exog <- rbind(mat_smape_exog,
                        colMeans(mat_smape_exog),                                 # Calculation of the Mean row.
                        colMeans(mat_smape_exog / apply(mat_smape_exog, 1, min))) # Calculation of the Mean percent deviation row.
rownames(mat_smape_exog) <- c(var_endog, 'Mean', 'Mean percent deviation')
colnames(mat_smape_exog) <- fcast_funcs_exog_names
